...
public class OntoUMLNameHelper {
	...
	public static String getTypeName(EObject elem)
	{
		...
		if(elem instanceof IntegerIntervalDimension) return "IntegerIntervalDimension";
		if(elem instanceof DecimalIntervalDimension) return "DecimalIntervalDimension";
		if(elem instanceof IntegerOrdinalDimension) return "IntegerOrdinalDimension";
		if(elem instanceof DecimalOrdinalDimension) return "DecimalOrdinalDimension";
		if(elem instanceof IntegerRationalDimension) return "IntegerRationalDimension";
		if(elem instanceof DecimalRationalDimension) return "DecimalRationalDimension";
		if(elem instanceof StringNominalStructure) return "StringNominalStructure";
		if(elem instanceof MeasurementDomain) return "MeasurementDomain";
	...
	public static String getUnknownType(RefOntoUML.Element element){
		String type = element.getClass().toString().substring(element.getClass().toString().lastIndexOf(".")+1,element.getClass().toString().length());
	    return type;
	}
